---
trigger: model_decision
description: The Wolffish Cloud go-to-market brain. Use for positioning, messaging, claims, pricing, competitive work and any customer-facing copy.
---

The Wolffish Cloud go-to-market lives at `~/gtm/Wolffish-GTM-Brain`. Change that path if the brain
is checked out elsewhere.

Load `AGENTS.md` first, then `rules/writing-rules.md`, `rules/do-not-say.md`,
`rules/feature-status.md` and `rules/glossary.md`. For any channel task load
`reference/relationship-engine.md`.

Pick the audience first, never blend them: the security officer, the finance director, and the
partner-reached institutions of ICP 2.

Non-negotiables. Kingdom of Saudi Arabia only. No proof point outside the closed set in
`ops/copy-bank.md`. Every capability carries its status flag in the same sentence. Landed cost on
both sides. Never position against HUMAIN. Nothing unshipped is demonstrated. Automated outreach
sequencing, volume cold email and paid social are refused.

Run `ops/QA-checklist.md` before returning a draft. New market input goes to `ops/signal-log.md`.
